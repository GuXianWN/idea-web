      ...
      <div class="w-[70vw] bg-white p-5 rounded-[15px] flex-col flex">
        ...
        <div class="mt-20 flex flex-col">
          <div class="w-[100%]">
            ...
            <el-input class="w-[21%] pb-2 mb-2 ml-2" v-model="inputTag" @keydown.enter.native="addTag"
                      size="small" placeholder="输入自定义标签,最多8个字符" maxlength="8" show-word-limit/>
            <el-button class="w-[5%] ml-2" size="small" type="primary"
                       @click="addTag">添加
            </el-button>
            <!--地区-->
            <span class="text-[15px] font-bold pb-4 ml-4">编辑地区</span>
            <el-input
                class="w-[22%] ml-2"
                size="small"
                v-model="key"
                placeholder="让你的文章更加适合你的地区"
                @focus.stop="show=true"
                @keyup.enter="querySearchAsync(key)"
                @input="timeOut"></el-input>
            <!--专栏-->
            <span class="text-[15px] font-bold pb-4 ml-4">所在专栏</span>
            <el-select class="w-[22%] ml-2" size="small" v-model="idea.programa" placeholder="请选择">
              <el-option
        ...
        <el-button class="mt-5 ml-[auto]" type="primary" @click="addIdea">发布</el-button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    addIdea() {
      ...
      this.postRequest('/idea/addIdea', this.idea).then(resp => {
        if (resp.code === 200) {
